...
void setup() {
  ...
  TZOffset = 0; // Default to offset of 0, which will be correct with RTC or reset by Serial input
  Serial.println("Now with TIMEZONES!!!");
...
unsigned long readSerialLong(int digits) {
  unsigned long num = 0;
  int i = 0;
  char c;
  ...
  if (digits == 0) {
    return 0;
  } else if (digits > 10) { // go for the max
    digits = 10;
  }

  // wait for more data if not as much as requested, but timeout eventually
  while (Serial.available() < digits && i++ < 50) {
    delay(10);
  }
...
void processSerialMessage() {
  ...
  while (Serial.available() >=  a5_COMM_MSG_LEN ) { // time message consists of a header and ten ascii digits
    ...
    if ( Serial.read() == a5_COMM_HEADER) {
      ...
      if ( c == 'S' )
      {
        if (c2 == 'T') // COMMAND: ST0123456789, SET EPOCH TIME
        {
          time_t pctime = 0;
          for ( i = 0; i < 10; i++) {
            c = Serial.read();
          ...
          setTime(pctime + TZOffset); // Sync Arduino clock to the time received on the serial port
          DisplayWord ("SYNCD", 500);
          DisplayWordDP("____2");
        ...
        else if (c2 == 'L') // COMMAND: SLyyyy-mm-ddThh:mm:ss - set Local time as ISO time string
        {
          int yyyy, mm, dd, h, m, s;
          yyyy = (unsigned int) readSerialLong(4);
          Serial.read();  // -
          mm = (unsigned int) readSerialLong(2);
          Serial.read();  // -
          dd = (unsigned int) readSerialLong(2);
          Serial.read();  // T
          h = (unsigned int) readSerialLong(2);
          Serial.read();  // :
          m = (unsigned int) readSerialLong(2);
          Serial.read();  // :
          s = (unsigned int) readSerialLong(2);

          // From Time library: API starts months from 1, this array starts from 0
          const uint8_t monthDays[] = {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};

          // apply minimal sanity constraints to values, 2000 years should work
          // Feb could be invalid in non-leap-years so don't do that
          yyyy = constrain(yyyy, 2000, 4000);
          mm = constrain(mm, 1, 12);
          dd = constrain(dd, 1, monthDays[mm - 1]);
          h = constrain(h, 0, 23);
          m = constrain(m, 0, 59);
          s = constrain(s, 0, 59);
          TZOffset = 0; // not necessary with an explicit local time

          DisplayWord(" SET ",500);
          setTime(h, m, s, dd, mm, yyyy);
          Serial.println("PC Local Time Sync Signal Received.");
          SerialPrintTime();
        ...
        else if (c2 == 'Z') // Set the timezone offset from -12:00 to 14:00
        {
          // timezone offset will be applied when the epoch is recieved via the ST command
          // it's a sloppy way of getting local time from a remote source for DST adjustments
          // better off using SL command to set an explicit local time and just leave TZOffset zero
          long timeoffset = 0;
          int minoffset = 0;
          char sign = Serial.read();  //get the +/- sign
          timeoffset = readSerialLong(2);
          Serial.read();  // get the :
          minoffset = readSerialLong(2);
          minoffset = constrain(minoffset, 0, 59);

          timeoffset = (timeoffset * 3600) + minoffset;
          if (sign == '-')
            timeoffset = timeoffset * -1;
          timeoffset = constrain(timeoffset, -12 * 3600, 14 * 3600);

          if (timeoffset != TZOffset) {
            setTime(now() - TZOffset + timeoffset); // Adjust Arduino time to the difference in timezones
            //            DisplayWord ("TZSET", 300);
            //            DisplayWordDP("_____");
            Serial.print("Time Zone Received. ");
    ...
    // If there's more data on the serial buffer that isn't the COMM_HEADER, just clear it out
    // Clears invalid or badly formed commands from the queue
    // This also cleans up noise from a connected ESP8266 that may arrive after a command
    while (Serial.available() > 0 && Serial.peek() != a5_COMM_HEADER) {
      Serial.read();
      delay(10);
    }
  }
